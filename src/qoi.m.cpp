...
constexpr PPMOutput readPPMFile(const std::filesystem::path& filename){
    ...
    for(int iter = 0; iter < width * height; ++iter){
        char rgb[3];
        file.read(rgb, 3);
        pixels[iter] = {.d_red=static_cast<Byte>(rgb[0]), .d_green=static_cast<Byte>(rgb[1]), .d_blue=static_cast<Byte>(rgb[2]), .d_alpha=255};
    }

    return {.d_width=width,.d_height=height,.d_channels=4,.d_colorspace=0,.d_pixels=pixels};
}
...
void printPixel(const RGBA& pixel){
    std::cout << "RED: " << static_cast<uint32_t>(pixel.d_red)
              ...
              << ", ALPHA: " << static_cast<uint32_t>(pixel.d_alpha)
              << std::endl;
}
    ...
    for(auto iter = ppmData.d_pixels.begin(); iter != ppmData.d_pixels.end(); iter++){
        ...
        if(indexArr[index] == *iter){
            ...
            if((dr >= -2 && dr <= 1) && (dg >= -2 && dg <= 1) && (db >= -2 && db <= 1)){
                encodedData.emplace_back(QOI_OP_DIFF | ((dr + 2) << 4) | ((dg + 2) << 2) | (db + 2));
...
void writeToPPMFile(const std::filesystem::path& filename, const DecodedOutput& decodedData){
    ...
    out << "P6\n" << decodedData.d_header.d_width
        << " " << decodedData.d_header.d_height << "\n255\n";
...
int main(int argc, char * argv[]){
    ...
    try
    {
        if(op == DECODE_OP){
            ...
            throw std::runtime_error("Invalid operation selected. Use either <encode> for qoi encoding or <decode> for qoi decoding.");
        }
    }
